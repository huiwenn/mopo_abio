...
def train(args=get_args()):
    ...
    #dataset = d4rl.qlearning_dataset(env) # make offline rl datset.     
    #args.obs_shape = env.observation_space.shape
    #args.action_dim = np.prod(env.action_space.shape)

    # TODO: load dataset and edit
    task = args.task
    dataset = np.load(f"../dataset/dataset_{task}.npz")
    ...
    # log, changed from tensorboard to wandb
    wandb.init(
        project=args.task,
        name=f"seed_{args.seed}_{args.algo_name}_{datetime.datetime.now().strftime('%m%d_%H%M%S')}",
        config=vars(args),
        dir=args.logdir
    )
    log_path = wandb.run.dir
    logger = Logger(writer=wandb, log_path=log_path)

    if args.device == 'cpu' or torch.cuda.is_available() == False:
        device = torch.device("cpu")
    else:
        device = torch.device(args.device) #give device as cuda:0
